...
src_unpack() {
	...
	if kernel_is ge 2 6 29; then
		ewarn "Kernel 2.6.29+ found, using experimental patch...."
		epatch "${FILESDIR}"/ati-acpi-2.6.29.patch || die "epatch failed"
        	if linux_chkconfig_present PARAVIRT; then
			epatch "${FILESDIR}"/ati-xen-irq.patch || die "epatch failed"
	        fi
	fi
